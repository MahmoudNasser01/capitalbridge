...
import { fundRequests as initialRequests } from "@/admin/mocks/listing-requests";
...
export function FundRequests() {
  ...
  const update = (id: string, status: ListingRequest["status"], verb: string) => {
    ...
    toast({ title: `Fund request ${verb}`, description: `${requests.find((r) => r.id === id)?.entityName} moved to ${status}.` });
  };
